/*
------------------------------------------------------------------------------ 
This code was generated by Amplication. 
 
Changes to this file will be lost if the code is regenerated. 

There are other ways to to customize your code, see this doc to learn more
https://docs.amplication.com/how-to/custom-code

------------------------------------------------------------------------------
  */
import { ArgsType, Field } from "@nestjs/graphql";
import { ApiProperty } from "@nestjs/swagger";
import { UserPostWhereInput } from "./UserPostWhereInput";
import { IsOptional, ValidateNested, IsInt } from "class-validator";
import { Type } from "class-transformer";
import { UserPostOrderByInput } from "./UserPostOrderByInput";

@ArgsType()
class UserPostFindManyArgs {
  @ApiProperty({
    required: false,
    type: () => UserPostWhereInput,
  })
  @IsOptional()
  @ValidateNested()
  @Field(() => UserPostWhereInput, { nullable: true })
  @Type(() => UserPostWhereInput)
  where?: UserPostWhereInput;

  @ApiProperty({
    required: false,
    type: [UserPostOrderByInput],
  })
  @IsOptional()
  @ValidateNested({ each: true })
  @Field(() => [UserPostOrderByInput], { nullable: true })
  @Type(() => UserPostOrderByInput)
  orderBy?: Array<UserPostOrderByInput>;

  @ApiProperty({
    required: false,
    type: Number,
  })
  @IsOptional()
  @IsInt()
  @Field(() => Number, { nullable: true })
  @Type(() => Number)
  skip?: number;

  @ApiProperty({
    required: false,
    type: Number,
  })
  @IsOptional()
  @IsInt()
  @Field(() => Number, { nullable: true })
  @Type(() => Number)
  take?: number;
}

export { UserPostFindManyArgs as UserPostFindManyArgs };
